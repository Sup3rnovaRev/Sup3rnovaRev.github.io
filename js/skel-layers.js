							...
							a.each(function() {
								...
								children.forEach(function(item){
									...
									if (item.nodeName == "#text"){
										// Normal text
										childrenstrings = childrenstrings.concat(item.nodeValue);
									}else if (item.nodeName == "SPAN"){
										// Icon span
										// Make sure the spans have unique id's
										if (typeof item.id !== 'undefined' && item.id != ''){
											var clone = item.cloneNode(true);
											clone.id = "_" + item.id 
											childrenstrings = childrenstrings.concat(clone.outerHTML);
										}else{
											childrenstrings = childrenstrings.concat(item.outerHTML);
										}
									}
								});

								b.push(
									'<a class="link depth-' + indent + '"' + ( (typeof href !== 'undefined' && href != '') ? ' href="' + href + '"' : '') + ( (typeof onClick !== 'undefined' && onClick != '') ? ' onclick="' + onClick + '"' : '') + '><span class="indent-' + indent + ' ' + '"></span>' + childrenstrings + '</a>'
								); 
